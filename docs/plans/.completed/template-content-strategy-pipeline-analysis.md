...
1. **W1 step 4 is already handled; W1 step 5 is broken.**
   ...
   - **Step 5 (build hierarchy):** the `-TriggerProjectSetup $True` path (default)
     ...
     incompatible with `gap-miner-v2-foxtrot91`'s new structure. The user passes
...
3. **NEW bug: AGENTS.md semantic rewrite silently fails.** The script targets
   ...
   replacement did land (`nam20485/gap-miner-v2-foxtrot91` → the clone name), but the
...
- **W1.5 (BROKEN — needs replacement):** `-TriggerProjectSetup $True` currently
    ...
    template and is incompatible with `gap-miner-v2-foxtrot91`'s new structure (the user
...
**In this template repo (`nam20485/gap-miner-v2-foxtrot91`):**
...
./scripts/create-repo-from-slug.ps1 `
  -Slug "gap-miner-v2" -TemplateRepoName "gap-miner-v2-foxtrot91" `
...
| Step | Description |
| --- | --- |
| Create repo | `gh repo create --template nam20485/gap-miner-v2-foxtrot91` |
...
| **Trigger follow-up workflow (W1.5) — BROKEN** | When `-TriggerProjectSetup $True` (default), the script calls `trigger-project-setup.ps1` which creates an `orchestration:dispatch` issue invoking `/orchestrate-dynamic-workflow $workflow_name = project-setup`. **Problem:** this orchestrator was ported from a different template and is largely incompatible with `gap-miner-v2-foxtrot91`'s new structure. The user's example invocation explicitly passes `-TriggerProjectSetup $False` to bypass it. Needs to be replaced with a dispatch that invokes `/gh-issue-tracking-init` directly (no args — its defaults resolve to the current repo + seeded `plan_docs/`). |
...
- **W1 step 5 (build hierarchy) trigger is broken.** `-TriggerProjectSetup $True`
  ...
  incompatible with `gap-miner-v2-foxtrot91`'s new structure. The user already bypasses
...
- **AGENTS.md rewrite bug (NEW, 2026-07-19).** The script targets
  ...
  (`nam20485/gap-miner-v2-foxtrot91` → the clone name), but the role label was not
