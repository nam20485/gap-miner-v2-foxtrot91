# Plan — Upstream fixes in `nam20485/gap-miner-v2-foxtrot91`

> Handoff document for an implementing agent. Source: the forensic analysis `gap-miner-v2-sierra46-forensic-analysis.md` (§9, §10) in the [`nam20485/gap-miner-v2-sierra46`](https://github.com/nam20485/gap-miner-v2-sierra46) repo. Target repo: [`nam20485/gap-miner-v2-foxtrot91`](https://github.com/nam20485/gap-miner-v2-foxtrot91) (the parent template that owns the `/gh-issue-tracking-init` skill as the canonical source of truth).

| | |
|---|---|
| **Plan** | Forensic-fixes upstream in gap-miner-v2-foxtrot91 |
| **Target repo** | `nam20485/gap-miner-v2-foxtrot91` |
...
A forensic analysis of the skill applied to `nam20485/gap-miner-v2-sierra46` found one **latent Int32-overflow bug**, two documentation gaps, and one missing level (`defect`). This plan fixes all of it **upstream** in `nam20485/gap-miner-v2-foxtrot91` so downstream repos inherit the improvements the next time they vendor the skill. Per-repo manual steps (adding Project views, tweaking Status options, adding plan-issue context) are out of scope.

---

## 1. Scope & Inputs

- **Target repo:** `nam20485/gap-miner-v2-foxtrot91` (the repo that owns the `/gh-issue-tracking-init` skill as its canonical source of truth)
- **Skill path in target:** `.agents/skills/gh-issue-tracking-init/`
- **Generic scripts path:** `scripts/` (the repo-root `common-auth.ps1`, `import-labels.ps1`, `create-milestones.ps1`)
...
- **Fixes applied upstream flow forward:** once W1–W4 merge into `gap-miner-v2-foxtrot91`, any repo cloned from the template afterward inherits them automatically.
...
1. Clone/fetch `nam20485/gap-miner-v2-foxtrot91` and `cd` into it.
...
The plan is done when, against `nam20485/gap-miner-v2-foxtrot91`:
...
4. [ ] A PR opened against `nam20485/gap-miner-v2-foxtrot91`'s default branch:
...
```text
You are implementing a fix plan in nam20485/gap-miner-v2-foxtrot91. The fix plan is:
  docs/plans/gap-miner-v2-foxtrot91-fix-plan.md
...
5. Open a PR against the default branch of nam20485/gap-miner-v2-foxtrot91:
