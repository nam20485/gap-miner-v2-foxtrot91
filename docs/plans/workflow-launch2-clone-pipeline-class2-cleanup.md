...
| **Template repo** | `nam20485/gap-miner-v2-foxtrot91` (consumer of the new path) |
| **Status** | Active |
| **Date** | 2026-07-20 |
...
When `gap-miner-v2-foxtrot91` is used as a GitHub template, the existing
...
For `gap-miner-v2-foxtrot91`-seeded instances, that dispatch body is incompatible: the
...
```pwsh
.SYNOPSIS
    Remove Class-2 template state from a freshly cloned gap-miner-v2-foxtrot91 instance.
...
```pwsh
.SYNOPSIS
    Trigger `/gh-issue-tracking-init` on a freshly seeded gap-miner-v2-foxtrot91 clone.
...
#### §A.3 — `create-repo-gap-miner-v2-foxtrot91.ps1` (new orchestrator)
...
```pwsh
.SYNOPSIS
    Create a new `gap-miner-v2-foxtrot91`-seeded repo with Class-2 cleanup and
...
.DESCRIPTION
    ...
    plus the gap-miner-v2-foxtrot91-specific ones (-TriggerHierarchyInit, default $true).
...
- Hard-codes `$TemplateRepoName = 'gap-miner-v2-foxtrot91'` and
...
Add a new test file `scripts/create-repo-gap-miner-v2-foxtrot91.Tests.ps1` covering:
...
- `create-repo-gap-miner-v2-foxtrot91.ps1`:
...
### §B — gap-miner-v2-foxtrot91 template changes (set-project-fields Phase guard)

Single-file patch in this repo (gap-miner-v2-foxtrot91), independent of §A:
...
**Step 2.** Implement §B in `gap-miner-v2-foxtrot91` (one-file patch + Pester case).

**Step 3.** Verify end-to-end by picking an existing slug (e.g. `gap-miner-v2`)
and invoking the new orchestrator:

```pwsh
./scripts/create-repo-gap-miner-v2-foxtrot91.ps1 `
...
- `AGENTS.md:5` in the template still reads "…**GitHub template repo**…"; in the
  ...
  `nam20485/gap-miner-v2-foxtrot91` GitHub template…" (end-to-end confirmation of
  W1.4's fix).
...
- §B Pester case green in `gap-miner-v2-foxtrot91`; existing skill tests stay green.
...
- **Backward compatibility preserved:** running the legacy entry point
  ...
  non-gap-miner-v2-foxtrot91 template) produces the exact same dispatch issue body
  (`/orchestrate-dynamic-workflow $workflow_name = project-setup`) as today.
- **gap-miner-v2-foxtrot91 clones are clean:** a clone created via
  `create-repo-gap-miner-v2-foxtrot91.ps1` has blank memory, no template plans, no
